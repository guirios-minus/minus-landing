...
export default function HeroSection() {
  ...
  return (
    <section className="pt-28 pb-20 px-5 bg-[#fafafa] relative overflow-hidden noise-bg">
      ...
      <div className="absolute bottom-0 left-[-60px] w-48 h-48 rounded-full bg-[#764ba2] opacity-10 blur-2xl pointer-events-none" />
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        ...
        <h1 className="font-space font-black leading-[1.05] tracking-tight text-[#0a0a0a] mb-6">
          ...
          <span className="block text-5xl md:text-7xl lg:text-8xl text-[#667eea] relative">
            ...
            <svg className="absolute -bottom-2 left-0 w-full" viewBox="0 0 400 12" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
              <path d="M2 9C60 3 140 1 220 5C300 9 360 3 398 6" stroke="#667eea" strokeWidth="5" strokeLinecap="round" strokeOpacity="0.4"/>
            </svg>
          </span>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          <a
            ...
            className="inline-flex items-center justify-center gap-2 bg-[#667eea] brutal-border brutal-shadow brutal-hover font-space font-black text-lg px-8 py-4 text-white"
          >
            {t('cta_primary')}
            <ArrowRight size={18} />
          </a>
        ...
        <div className="mt-16 brutal-border brutal-shadow-lg bg-white overflow-hidden">
          ...
          <div className="p-4 md:p-8 bg-gradient-to-br from-[#f5f6ff] to-[#fafafa] min-h-[260px] flex gap-4 overflow-hidden">
            {[
              ...
              { label: 'Em Progresso', color: '#764ba2', count: 2 },
              { label: 'Concluído', color: '#0a0a0a', count: 5 },
            ].map((col) => (
              <div key={col.label} className="flex-1 min-w-0">
                <div
                  ...
                  style={{ backgroundColor: col.color + '15', borderColor: col.color, color: col.color }}
                >
                  {col.label} · {col.count}
                </div>
                <div className="flex flex-col gap-2">
                  {Array.from({ length: col.count }).map((_, i) => (
                    <div key={i} className="bg-white brutal-border brutal-shadow-sm p-2.5">
